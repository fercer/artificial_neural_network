...
public:
	...
	double getWeightedInputWithDerivatives(const unsigned long long src_current_network_time);
...
private:
	...
	*/
	...
	and should be accessed exclusively in their position [0]:
	*/
	double ** self_weights_pointers_manager;
	double *** self_weights_derivatives_pointers_manager;

	/* These pointers contain the weights and derivatives values of the entire network,
	If the memory is self allocated, these pointers point directly to the pointers 'self_weights_pointers_manager' and 'self_weights_derivatives_pointers_manager' respectively,
	and should be accessed exclusively in their position [0]:
	*/
	...
	double(Weight_node::*get_input_response_with_derivatives) ();
	double(Weight_node::*get_input_response) ();
	double getWeightedInputWithDerivatives();
